...
* Section 2: Data Analysis
  ...
  * Section 2.3: Rating Vs Metascore
...
This section will have 5 subsections: *Genre's Average rating, Director’s Effect on Rating and Metascore, Rating Vs Metascore, Runtime’s Effect on Rating and Metascore, and Rating’s and Metascore’s Effect on Revenue*. Each subsection will illustrate how `Runtime`, `Rating`, `Revenue`, `Metascore`and `Director` affects a movie's box office success and creates bias reviews.
...
```
Based on average `Ratings` and highest frequency of movies, ***Christopher Nolan*** is the most frequent director with the highest `Ratings` from his 5 movies: *The Dark Knight, Inception	, Interstellar, The Prestige, and The Dark Knight Rises*. The `Director` ranked 9th in the data frame, **Director's Movie Count (n) w/ Highest Ratings**, and 2nd in the data frame, **Director's Average Rating**.

However, one of the movies (***The Dark Knight Rises***) is a sequel of the other and is based on a very popular *Marvel* icon, *Batman*, which could have compropmised the legitamacy of the `Ratings`.
```{r}
...
* ***Drama*** has the **5th** highets Rating based on `Genre` (**6.953801**
)
...
```
Therefore, the most successful `Director` based on frequncy of highly rated movies is ***Christian Bale***. The reasons for his success is that one movie is a sequel of a previously successful movie, his `Genre` choice, and the `Actors` who played a role in the movies.  
...
Usually in the first week of a movie's premiere, `Ratings` and `Metascores` are given by first-time viewers who want to give their opinion  of the quality of the movie. Besides the advertisments and who the `Actors` and the `Directors` are, `Ratings` and `Metscores` can greatly affect how much viewership and `Revenue` a movie gains in the box office, since positive reviews encourage people to buy tickets while negative reviews causes people to steer clear from the movie theaters.
```{r}
...
```
In  the scatter plots, **Rating Vs Revenue Scatterplot**,`Revenue` exponetially grows as its `Ratings` rise. However, in the scatter plot **Metascore Vs Revenue Scatterplot**, `Revenue` has a logistic growth as `Metascore` increases. `Revenue` reaches a `Revenue` plateau of around ***$100 million*** with a `Metascore` range ***from 70 to 100***.

Therefore, higher `Ratings` (in an exponential sense) and higher `Metascores` (in a logistic sense), will lead to higher `Revenue` and viewerships in the box office.
...
`Ratings` and `Metascores` can greatly conflict with each other. The highest `Ratings` are more prone to conlict with its `Metascores`, but the highest `Metascores` is less likely to conflict with its `Ratings`.
...
During the first week of a movie’s premiere, `Ratings` and `Metascores` are distributed and can greatly affect the box office success and `Revenue` of a movie. `Ratings` cause an exponetial growth for a movie's `Revenue`, while `Metascores` can cause a logistic growth for a movie's `Revenue`. 

A movie's `Rating`, `Director`, `Actors`, `Genre`, `Metascore`, `Runtime`, and `Revenue` play a direct role for a movie's success and can increase positive feedback. However, these factors can create biasness that deter the integrity of the `Rating` and `Metascore`. 
